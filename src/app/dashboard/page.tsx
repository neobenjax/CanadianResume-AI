        ...
        <div className="space-y-8">
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-slate-400 mt-1">Manage and tailor your resumes for different applications.</p>
                </div>
            ...
            {!resumes || resumes.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-20 bg-slate-900/50 backdrop-blur-sm rounded-3xl border border-dashed border-slate-700">
                    <div className="w-16 h-16 bg-slate-800 rounded-full flex items-center justify-center mb-4 border border-slate-700">
                        <FileText className="w-8 h-8 text-primary-400" />
                    </div>
                    <h3 className="text-xl font-semibold text-white mb-2">No resumes yet</h3>
                    <p className="text-slate-400 max-w-md text-center mb-6">Create your first resume by using your profile data. You can tailor it for specific jobs later.</p>
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {resumes.map((resume) => (
                        <GlassCard
                            key={resume.id}
                            hoverEffect
                            className="relative flex flex-col h-full cursor-pointer group bg-slate-900 border-slate-700/50"
                            onClick={() => router.push(`/dashboard/resumes/${resume.id}`)}
                        >
                            <div className="flex justify-between items-start mb-4">
                                <div className="w-10 h-10 bg-primary-900/40 rounded-lg flex items-center justify-center text-primary-400 border border-primary-500/20">
                            ...
                            <h3 className="text-lg font-bold text-white line-clamp-1 mb-1">{resume.title}</h3>
                            <p className="text-sm text-slate-400 mb-6">Target: General Application</p>
